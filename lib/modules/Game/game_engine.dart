...
class _GameEngine implements GameEngine {
  ...
  Future<UiState> reportMove(int row, int col) async {
    _uiState = computeNewState(_UiState.toSquareIndex(row, col), Player.human);
    return _uiState;
  }
  ...
  _UiState computeNewState(int squareIndex, Player player) {
  ...
  _UiState computeDashMove() {
    ...
    return computeNewState(nextMove, Player.dash);
  }
